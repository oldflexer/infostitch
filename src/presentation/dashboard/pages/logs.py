...
async def render_logs(
    ...
    if st.session_state.logs_auto_refresh:
        st_autorefresh(
            interval=st.session_state.logs_refresh_interval * 1000,
...
def _render_log_filters() -> None:
    ...
    with col2:
        module = st.text_input(
            "Module",
            placeholder="e.g. pipeline.steps.fetch_rss",
            key="log_filter_module")

    with col3:
        correlation_id = st.text_input(
            "Correlation ID",
            placeholder="e.g. a1b2c3d4",
            key="log_filter_correlation")
    ...
    st.session_state.logs_filters = {
        ...
        "start_time": datetime.combine(
            start_date,
            datetime.min.time()) if start_date else None,
        "end_time": datetime.combine(
            end_date,
            datetime.max.time()) if end_date else None,
    }
...
def _render_log_detail(log: Any) -> None:
    ...
    with st.expander("📄 Log Detail", expanded=True):
        ...
        with col2:
            st.write(
                f"**Correlation ID:** {
                    log.context_json.get(
                        'correlation_id',
                        'N/A') if log.context_json else 'N/A'}")
...
async def _render_log_stats(log_repo: Any) -> None:
    ...
    if by_level:
        ...
        fig = px.bar(level_df, x="Level", y="Count",
                     color="Level", title="Logs by Level (24h)")
    ...
    if by_module:
        ...
        fig = px.bar(module_df, x="Module", y="Count",
                     title="Top Modules by Log Count")
        st.plotly_chart(fig, use_container_width=True)
    st.divider()
