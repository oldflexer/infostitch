...
async def _render_rss_sources_tab(source_repo: Any) -> None:
    ...
    with st.expander("➕ Add New RSS Source", expanded=False):
        with st.form("add_rss_source"):
            url = st.text_input(
                "RSS URL", placeholder="https://example.com/feed.xml")
    ...
    for source in sources:
        with st.container():
            ...
            with col1:
                ...
                st.caption(
                    f"Created: {source.created_at.strftime('%Y-%m-%d')}")
            with col2:
            ...
            if st.session_state.get(f"edit_channel_{channel.id}", False):
                with st.form(f"edit_channel_form_{channel.id}"):
                    ...
                    new_config = st.text_area(
                        "Config (JSON)", value=json.dumps(
                            channel.config, indent=2), height=150)
    ...
    with st.expander("➕ Add New LLM Model", expanded=False):
        with st.form("add_llm_model"):
            ...
            model_id = st.text_input(
                "Model ID", placeholder="gemini-1.5-flash-latest")
            api_key_ref = st.text_input(
                "API Key Ref (env var name)", placeholder="GEMINI_API_KEY")
