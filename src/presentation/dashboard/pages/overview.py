...
async def render_overview(
    ...
    st.markdown('<div class="main-header">📊 Overview</div>',
...
async def _get_recent_runs(
        post_repo: SqlAlchemyPostRepository, limit: int = 10) -> List[Dict[str, Any]]:
...
def _render_pipeline_status(
        db_settings: Dict[str, Any], recent_runs: List[Dict[str, Any]]) -> None:
...
def _render_quick_stats(
    ...
    with col1:
        ...
        st.metric("RSS Sources", f"{enabled_sources}/{len(sources)}",
                  delta=f"{len(sources) - enabled_sources} disabled")

    with col2:
        enabled_channels = sum(1 for c in channels if c.enabled)
        st.metric("Channels", f"{enabled_channels}/{len(channels)}",
                  delta=f"{len(channels) - enabled_channels} disabled")
...
def _render_recent_posts(posts: List[Any]) -> None:
    ...
    for post in posts[:5]:
        with st.expander(f"{post.title[:80]}... ({post.created_at.strftime('%Y-%m-%d %H:%M')})"):
            st.write(post.summary[:200] +
                     "..." if len(post.summary) > 200 else post.summary)
