...
async def render_metrics(
    ...
    st.markdown('<div class="main-header">📈 Metrics</div>', unsafe_allow_html=True)
    ...
    # Charts in tabs
    tab1, tab2, tab3, tab4 = st.tabs(["📊 Pipeline", "📈 Articles", "🔌 API", "🎯 Deduplication"])
...
def _render_pipeline_charts(df: pd.DataFrame) -> None:
    ...
    fig = px.line(daily_counts, x='date', y='count', title='Posts Published per Day')
    ...
    with st.container():
        ...
        if 'channel_id' in df.columns:
            ...
            fig = px.bar(channel_counts, x='channel_id', y='count', title='Posts by Channel')
            st.plotly_chart(fig, use_container_width=True)
    ...
    with st.container():
        ...
        if 'template_id' in df.columns:
            ...
            fig = px.pie(template_counts, values='count', names='template_id', title='Posts by Template')
            st.plotly_chart(fig, use_container_width=True)
...
def _render_article_charts(df: pd.DataFrame) -> None:
    ...
    with col1:
        ...
        if 'is_duplicate' in df.columns:
            ...
            dup_counts['is_duplicate'] = dup_counts['is_duplicate'].map({True: 'Duplicate', False: 'Unique'})
            fig = px.pie(dup_counts, values='count', names='is_duplicate', title='Duplicate vs Unique Posts')
            st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        ...
        if 'source_id' in df.columns:
            ...
            fig = px.bar(source_counts, x='source_id', y='count', title='Posts by Source')
            st.plotly_chart(fig, use_container_width=True)
...
def _render_deduplication_charts(df: pd.DataFrame) -> None:
    ...
    with col2:
        if 'source_id' in df.columns and 'is_duplicate' in df.columns:
            dup_by_source = df.groupby('source_id')['is_duplicate'].mean().reset_index()
            dup_by_source.columns = ['source_id', 'duplicate_rate']
            dup_by_source['duplicate_rate'] = dup_by_source['duplicate_rate'] * 100
            fig = px.bar(dup_by_source, x='source_id', y='duplicate_rate', title='Duplicate Rate by Source (%)')
            st.plotly_chart(fig, use_container_width=True)
