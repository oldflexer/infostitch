    ...
    async def run(self, context: PipelineContext) -> PipelineContext:
        ...
        with MetricsContext(pipeline_duration_seconds, {"dummy": "pipeline"}) as timer:
            ...
            for step in self._steps:
                ...
                step_timer = pipeline_step_duration_seconds.labels(
                    step=step_name).time()
                ...
                try:
                    ...
                    logger.info("Pipeline step completed",
                                step=step_name, correlation_id=correlation_id)
                    ...
                    logger.warning(
                        "Continuing pipeline after step failure",
                        step=step_name,
                        correlation_id=correlation_id)
                finally:
        ...
        logger.info("Pipeline completed", correlation_id=correlation_id,
                    status=status, errors=len(context.errors))
