...
class TestFullPipelineE2E:
    ...
    def mock_feedparser(self):
        ...
        with patch("application.pipeline.steps.fetch_rss.feedparser.parse") as mock:
            ...
            mock_feed.entries = [{"title": "AI Breakthrough in Machine Learning",
    ...
    def mock_publisher_clients(self):
        ...
        with patch("infrastructure.clients.telegram_client.TelegramClient") as mock_tg, \
            ...
            mock_tg_instance.send_photo = AsyncMock(
                return_value={"message_id": 1, "ok": True})
            ...
            mock_max_instance.send_message = AsyncMock(
                return_value={"message_id": 1, "ok": True})
            mock_max_instance.send_photo = AsyncMock(
                return_value={"message_id": 1, "ok": True})
    ...
    async def test_full_pipeline_dry_run(
        ...
        assert result.metrics.get("total_fetched", 0) == 6
    ...
    @pytest.mark.asyncio
    async def test_pipeline_empty_rss_feeds(
            self, full_pipeline, mock_rss_sources):
    ...
    @pytest.mark.asyncio
    async def test_pipeline_all_duplicates_filtered(
            self, full_pipeline, mock_rss_sources):
    ...
    async def test_pipeline_integration_all_steps(
        ...
        for step in original_steps:
            ...
            async def tracking_execute(
                    ctx, name=step_name, orig=original_execute):
