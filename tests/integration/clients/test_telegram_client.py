...
from infrastructure.clients.telegram_client import MockTelegramClient, TelegramClient


class TestTelegramClient:
    """Tests for real TelegramClient with mocked HTTP."""

    @pytest.fixture
    def telegram_client(self):
        """Create TelegramClient with test credentials."""
        return TelegramClient(bot_token="test-token", chat_id="test-chat-id")

    @pytest.fixture
    def mock_httpx_client(self):
        """Mock httpx.AsyncClient."""
        with patch("infrastructure.clients.telegram_client.httpx.AsyncClient") as mock:
            mock_instance = AsyncMock()
            mock.return_value = mock_instance
            yield mock_instance

    @pytest.mark.asyncio
    async def test_send_message_success(self, telegram_client, mock_httpx_client):
        """Test successful message sending."""
        mock_response = MagicMock()
        mock_response.json.return_value = {
            "ok": True,
            "result": {"message_id": 123, "chat": {"id": "test-chat-id"}},
        }
        mock_response.raise_for_status = MagicMock()
        mock_httpx_client.post.return_value = mock_response

        result = await telegram_client.send_message("Hello, World!")
    ...
    @pytest.mark.asyncio
    async def test_send_message_with_custom_params(self, telegram_client, mock_httpx_client):
    ...
    @pytest.mark.asyncio
    async def test_send_message_api_error(self, telegram_client, mock_httpx_client):
        ...
        with pytest.raises(RuntimeError, match="Telegram API error: Bad Request: chat not found"):
            await telegram_client.send_message("Test")

    @pytest.mark.asyncio
    async def test_send_message_http_error(self, telegram_client, mock_httpx_client):
    ...
    @pytest.mark.asyncio
    async def test_send_photo_success(self, telegram_client, mock_httpx_client):
        ...
        call_args = mock_httpx_client.post.call_args
        assert call_args[1]["json"]["chat_id"] == "test-chat-id"
        assert call_args[1]["json"]["photo"] == "https://example.com/photo.jpg"
        assert call_args[1]["json"]["caption"] == "Photo caption"
        assert call_args[1]["json"]["parse_mode"] == "HTML"

    @pytest.mark.asyncio
    async def test_send_photo_without_caption(self, telegram_client, mock_httpx_client):
        ...
        result = await telegram_client.send_photo("https://example.com/photo.jpg")
    ...
    async def test_send_photo_api_error(self, telegram_client, mock_httpx_client):
        """Test handling of Telegram API error for photo."""
        mock_response = MagicMock()
        mock_response.json.return_value = {
            "ok": False,
        ...
        with pytest.raises(RuntimeError, match="Telegram API error: Bad Request: wrong file identifier"):
            await telegram_client.send_photo("https://example.com/bad.jpg")

    @pytest.mark.asyncio
    async def test_send_document_success(self, telegram_client, mock_httpx_client):
        """Test successful document sending."""
        mock_response = MagicMock()
        mock_response.json.return_value = {
            "ok": True,
            "result": {"message_id": 999, "document": {"file_id": "doc123"}},
        }
        mock_response.raise_for_status = MagicMock()
        mock_httpx_client.post.return_value = mock_response

        result = await telegram_client.send_document(
        ...
        call_args = mock_httpx_client.post.call_args
        assert call_args[1]["json"]["chat_id"] == "test-chat-id"
        assert call_args[1]["json"]["document"] == "https://example.com/document.pdf"
        assert call_args[1]["json"]["caption"] == "Document caption"

    @pytest.mark.asyncio
    async def test_get_me_success(self, telegram_client, mock_httpx_client):
    ...
    @pytest.mark.asyncio
    async def test_get_me_http_error(self, telegram_client, mock_httpx_client):
    ...
    @pytest.mark.asyncio
    async def test_close(self, telegram_client, mock_httpx_client):
        """Test closing the client."""
        _ = telegram_client.client
    ...
    async def test_custom_base_url(self):
        ...
        client = TelegramClient(
            bot_token="123:ABC",
            chat_id="456",
            base_url="https://custom.api.telegram.org/bot",
        )
        assert client._base_url == "https://custom.api.telegram.org/bot123:ABC"


class TestMockTelegramClient:
    """Tests for MockTelegramClient."""

    @pytest.fixture
    def mock_client(self):
        """Create MockTelegramClient."""
