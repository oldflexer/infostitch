...
class TestDeduplicateStep:
    ...
    def mock_dedup_service(self):
        ...
        service.filter_by_jaccard = MagicMock(
            side_effect=lambda articles, recent_titles: [
                a for a in articles if a.title not in recent_titles])
        return service

    @pytest.mark.asyncio
    async def test_deduplicate_url_stage(
            self, deduplicate_step, sample_articles, mock_dedup_service):
    ...
    @pytest.mark.asyncio
    async def test_deduplicate_jaccard_stage(
            self, deduplicate_step, sample_articles, mock_dedup_service):
        ...
        context.get_recent_titles = lambda limit=10: [
            "AI News: New Model Released"]

        result = await deduplicate_step.execute(context)

        # After URL dedup: 3 articles (articles 1, 2, 3)
        # After Jaccard: 1 article removed (articles 1 & 2 have same title as
        # recent)
        assert len(result.deduplicated_articles) == 1
        assert result.metrics["after_jaccard_dedup"] == 1

    @pytest.mark.asyncio
    async def test_deduplicate_empty_input(
            self, deduplicate_step, mock_dedup_service):
    ...
    @pytest.mark.asyncio
    async def test_deduplicate_all_duplicates(
            self, deduplicate_step, mock_dedup_service):
