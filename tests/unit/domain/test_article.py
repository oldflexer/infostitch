...
<rss version="2.0">
    ...
    def test_article_equality(self):
        ...
        article1 = Article(id=1, title="Title 1", url=url, summary="Summary 1",
                           published_at=now, source_id=1, fetched_at=now)
        article2 = Article(id=1, title="Title 1", url=url, summary="Summary 1",
                           published_at=now, source_id=1, fetched_at=now)
        article3 = Article(id=2, title="Title 2", url=url, summary="Summary 2",
                           published_at=now, source_id=1, fetched_at=now)
    ...
    def test_clean_url_property(self):
        ...
        article = Article(
            ...
            title="Test",
            url=URL.from_string(
                "https://example.com/path?utm_source=test#fragment"),
            summary="Test",
            published_at=datetime.now(timezone.utc),
            source_id=1,
        )
